...
| **미래 예측력** | **AUROC 0.776** (2024년 검증 기준, 보수적 필터링 및 포아송 회귀 모델 적용) |
| **공간 전이성(Transferability)** | **AUROC 0.9105** (서울 학습 모델을 부산광역시에 적용한 Zero-shot 예측 결과) |
| **실용성 지표(PAI)**| **PAI@10% = 2.38 (서울)** / **PAI@10% = 5.64 (부산)** (상위 10% 위험 구역 관리 시 부산 전체 사고의 약 56.4% 예방 가능) |
...
| 지표 | 결과 | 의미 (논문 디펜스 포인트) |
|---|---|---|
| **미래 예측 AUROC** | **0.776** | 시간적 홀드아웃 검증 및 포아송 회귀(Poisson Regression) 도입으로 사고 횟수 예측 정밀도 향상. |
| **전체 기간 AUROC** | **0.839** | 전체 기간 기준 강건한 분류 성능. |
| **타 도시 전이성 (Busan)** | **0.9105** | 서울 데이터로만 학습된 모델이 부산광역시의 위험 지역을 완벽하게 예측. **공간적 일반화 성능(Spatial Generalization)** 입증. |
| **PAI@10% (서울)** | **2.38** | 상위 10% 위험 격자 단속 시, 2024년 서울 PM 사고의 약 **23.8%** 사전 예방 가능. |
| **PAI@10% (부산)** | **5.64** | 상위 10% 위험 격자 단속 시, 부산광역시 전체 PM 사고의 무려 **56.4%** 사전 예방 가능. 완벽한 지리적 타겟팅 입증. |
| **PAI@20% (서울)** | **2.28** | 상위 20% 위험 격자 단속 시, 2024년 서울 PM 사고의 약 **45.6%** 사전 예방 가능. |
