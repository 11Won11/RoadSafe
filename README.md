...
| **공간 전이성(Transferability)** | **AUROC 0.9089** (서울 학습 모델을 부산광역시에 적용한 Zero-shot 예측 결과) |
| **실용성 지표(PAI)**| **PAI@10% = 2.46 (서울)** / **PAI@10% = 5.99 (부산)** (상위 10% 위험 구역 관리 시 부산 전체 사고의 약 60% 예방 가능) |
...
| **타 도시 전이성 (Busan)** | **0.9089** | 서울 데이터로만 학습된 모델이 부산광역시의 위험 지역을 완벽하게 예측. **공간적 일반화 성능(Spatial Generalization)** 입증. |
| **PAI@10% (서울)** | **2.46** | 상위 10% 위험 격자 단속 시, 2024년 서울 PM 사고의 약 **25%** 사전 예방 가능. |
| **PAI@10% (부산)** | **5.99** | 상위 10% 위험 격자 단속 시, 부산광역시 전체 PM 사고의 무려 **59.9%** 사전 예방 가능. 완벽한 지리적 타겟팅 입증. |
| **PAI@20% (서울)** | **2.20** | 상위 20% 위험 격자 단속 시, 2024년 서울 PM 사고의 약 **44%** 사전 예방 가능. |
